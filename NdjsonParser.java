        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (root.has("testStepFinished")) {
                    ...
                    if (isHook) {
                        // hook step — only care if it failed, capture error on scenario
                        if ("FAILED".equalsIgnoreCase(result.path("status").asText())) {
                            JsonNode exception = result.path("exception");
                            String   errorMsg  = exception.isMissingNode()
                                    ? result.path("message").asText(null)
                                    : exception.path("message").asText(null);
                        ...
                        // exception objekt obsahuje message aj stackTrace oddelene
                        JsonNode exception = result.path("exception");
                        String errorMsg, stackTrace;
                        if (!exception.isMissingNode()) {
                            // Cucumber 7+ — exception má message a stackTrace zvlášť
                            errorMsg   = exception.path("message").asText(null);
                            stackTrace = exception.path("stackTrace").asText(null);
                        } else {
                            // Fallback — starší formát, všetko v message poli
