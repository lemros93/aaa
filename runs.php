...
<div class="table-wrap" id="run-<?= $r['id'] ?>" style="margin-bottom:20px">
    ...
    <!-- Features expand area — pure div, no table-row -->
        ...
        html += `<tr>
            <td style="width:32px">
                <button class="expand-btn" id="fbtn-${f.id}"
                    onclick="toggleFeat(this,${f.id})">+</button>
            </td>
            <td>
        ...
        html += `<tr>
            <td style="width:32px">
                <button class="expand-btn" id="sbtn-${s.id}"
                    onclick="toggleScen(this,${s.id})">+</button>
            </td>
            <td>
...
async function loadSteps(scenId) {
    ...
    data.forEach(st => {
        html += `<tr style="border-bottom:1px solid var(--border)">
            ...
            <td style="padding:8px 10px; vertical-align:top">
                ${st.screenshot_url ? `<img class="thumb" src="${escHtml(st.screenshot_url)}" onclick="showImage('${escHtml(st.screenshot_url)}')" alt="">` : ''}
                ${st.failed_screenshot_url ? `<img class="thumb" src="${escHtml(st.failed_screenshot_url)}" onclick="showImage('${escHtml(st.failed_screenshot_url)}')" alt="" style="border-color:var(--danger)">` : ''}
            </td>
        </tr>`;
    });
    html += '</tbody></table>';
