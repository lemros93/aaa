...
public class DatabaseWriter {
    ...
    private long insertRun(TestRun run, ImportConfig config) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setString(9,    config.environment);
            ps.setString(10,   config.appUrl);
            ps.setString(11,   config.buildNumber);
            ps.setString(12,   config.parallelMode);
            ps.setInt(13,      config.threadCount);
            ps.setBoolean(14,  config.jiraReporting);
            ps.setBoolean(15,  config.jiraNewExecution);
            ps.setString(16,   config.jiraCycleIds);
            ps.setString(17,   config.cucumberTags);
            ps.setString(18,   config.testedFeature);
            ps.setString(19,   config.testProperties);
